...
//
//	  2000-MAR-07 - Thanks to Ullrich Poll鄣ne for catching a range bug in one
...
inline SS_NOTHROW size_t sslen(const std::string& s)
{
	return s.length();
}
inline SS_NOTHROW size_t sslen(const std::wstring& s)
{
	return s.length();
}
...
public:
	...
	// They also are compiled if you have explicitly turned off refcounting
		...
		MYTYPE& assign(const MYTYPE& str, MYSIZE nStart, MYSIZE nChars)
		{
			...
			// value.  Thanks to Ullrich Poll鄣ne for catching this bug
	...
	#define STD_BUF_SIZE		2048 //elan:由原來1024改為2048,已避免當內容過長時,over buff size而不顯示之情形
